...
extension UIView {
    public static func animationDurationFactor() -> Double {
...
public func makeSpringBounceAnimation(
    _ keyPath: String,
...
extension UIColor {
    ...
    public func withMultipliedBrightnessBy(_ factor: CGFloat) -> UIColor {
        var hue: CGFloat = 0.0
        var saturation: CGFloat = 0.0
        var brightness: CGFloat = 0.0
        var alpha: CGFloat = 0.0
    ...
    public func withMultiplied(hue: CGFloat, saturation: CGFloat, brightness: CGFloat) -> UIColor {
    ...
    public func mixedWith(_ other: UIColor, alpha: CGFloat) -> UIColor {
    ...
    public func blitOver(_ other: UIColor, alpha: CGFloat) -> UIColor {
        let alpha = min(1.0, max(0.0, alpha))
    ...
    public func withMultipliedAlpha(_ alpha: CGFloat) -> UIColor {
        var r1: CGFloat = 0.0
        var g1: CGFloat = 0.0
        var b1: CGFloat = 0.0
        var a1: CGFloat = 0.0
    ...
    public func interpolateTo(_ color: UIColor, fraction: CGFloat) -> UIColor? {
    ...
    private var colorComponents: (r: Int32, g: Int32, b: Int32) {
        ...
        if self.getRed(&r, green: &g, blue: &b, alpha: nil) {
            return (
                Int32(max(0.0, r) * 255.0), Int32(max(0.0, g) * 255.0), Int32(max(0.0, b) * 255.0)
            )
        } else if self.getWhite(&r, alpha: nil) {
            return (
                Int32(max(0.0, r) * 255.0), Int32(max(0.0, r) * 255.0), Int32(max(0.0, r) * 255.0)
            )
        }
        return (0, 0, 0)
    }

    public func distance(to other: UIColor) -> Int32 {
    ...
    public static func average(of colors: [UIColor]) -> UIColor {
        var sr: CGFloat = 0.0
        var sg: CGFloat = 0.0
        var sb: CGFloat = 0.0
        var sa: CGFloat = 0.0
        ...
        return UIColor(
            red: sr / CGFloat(colors.count),
            green: sg / CGFloat(colors.count),
            blue: sb / CGFloat(colors.count),
            alpha: sa / CGFloat(colors.count)
        )
    }
}
...
extension CGSize {
    ...
    public func aspectFittedOrSmaller(_ size: CGSize) -> CGSize {
        let scale = min(
            1.0,
            min(size.width / max(1.0, self.width), size.height / max(1.0, self.height))
        )
        return CGSize(width: floor(self.width * scale), height: floor(self.height * scale))
    }
...
extension UIImage {
    public func precomposed() -> UIImage {
        ...
        if self.capInsets != UIEdgeInsets() {
            return result.resizableImage(
                withCapInsets: self.capInsets,
