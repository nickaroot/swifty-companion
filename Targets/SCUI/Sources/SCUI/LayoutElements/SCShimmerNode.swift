...
final class ShimmerEffectForegroundNode: ASDisplayNode {
    ...
    func update(backgroundColor: UIColor, foregroundColor: UIColor, horizontal: Bool = false) {
        if let currentBackgroundColor = self.currentBackgroundColor,
            currentBackgroundColor.isEqual(backgroundColor),
            let currentForegroundColor = self.currentForegroundColor,
            currentForegroundColor.isEqual(foregroundColor), self.currentHorizontal == horizontal
        {
            return
        }
        ...
        if horizontal {
            image = generateImage(
                CGSize(width: 320.0, height: 16.0),
                opaque: false,
                scale: 1.0,
            ...
            image = generateImage(
                CGSize(width: 16.0, height: 320.0),
                opaque: false,
                scale: 1.0,
                rotatedContext: { size, context in
                    context.clear(CGRect(origin: CGPoint(), size: size))
    ...
    func updateAbsoluteRect(_ rect: CGRect, within containerSize: CGSize) {
        if let absoluteLocation = self.absoluteLocation,
            absoluteLocation.0 == rect && absoluteLocation.1 == containerSize
        {
            return
        }
        ...
        if frameUpdated {
            self.imageNodeContainer.frame = CGRect(
    ...
    private func updateAnimation() {
        let shouldBeAnimating =
            self.isCurrentlyInHierarchy && self.absoluteLocation != nil
            && self.currentHorizontal != nil
    ...
    private func addImageAnimation() {
        guard let containerSize = self.absoluteLocation?.1, let horizontal = self.currentHorizontal
        else {
            return
        }
        ...
        if horizontal {
            ...
            self.imageNode.frame = CGRect(
                origin: CGPoint(x: -gradientHeight, y: 0.0),
                size: CGSize(width: gradientHeight, height: containerSize.height)
            )

            let animation = self.imageNode.layer.makeAnimation(
                from: 0.0 as NSNumber,
                to: (containerSize.width + gradientHeight) as NSNumber,
                keyPath: "position.x",
            ...
            self.imageNode.frame = CGRect(
                origin: CGPoint(x: 0.0, y: -gradientHeight),
                size: CGSize(width: containerSize.width, height: gradientHeight)
            )

            let animation = self.imageNode.layer.makeAnimation(
                from: 0.0 as NSNumber,
                to: (containerSize.height + gradientHeight) as NSNumber,
                keyPath: "position.y",
                timingFunction: CAMediaTimingFunctionName.easeOut.rawValue,
                duration: 1.3 * 1.0,
                delay: 0.0,
...
public final class ShimmerEffectNode: ASDisplayNode {
    ...
    public func update(
        ...
        shimmeringColor: UIColor,
        shapes: [Shape],
        horizontal: Bool = false,
        size: CGSize
    ) {
        if self.currentShapes == shapes, let currentBackgroundColor = self.currentBackgroundColor,
            currentBackgroundColor.isEqual(backgroundColor),
            let currentForegroundColor = self.currentForegroundColor,
            currentForegroundColor.isEqual(foregroundColor),
            let currentShimmeringColor = self.currentShimmeringColor,
            currentShimmeringColor.isEqual(shimmeringColor), horizontal == self.currentHorizontal,
        ...
        self.effectNode.update(
            backgroundColor: foregroundColor,
            foregroundColor: shimmeringColor,
            horizontal: horizontal
        )

        self.foregroundNode.image = generateImage(
            size,
            rotatedContext: { size, context in
                context.setFillColor(backgroundColor.cgColor)
                ...
                for shape in shapes {
                    ...
                    case let .roundedRectLine(startPoint, width, diameter):
                        ...
                        context.fill(
                            CGRect(
                                origin: CGPoint(x: startPoint.x + diameter / 2.0, y: startPoint.y),
                                size: CGSize(width: width - diameter, height: diameter)
                            )
                        )
